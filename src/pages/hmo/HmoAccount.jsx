...
function HmoAccount() {
  ...
  useEffect(() => {
    document.title = import.meta.env.VITE_COMPANY_NAME + " | HMO Account";
  }, []);
...
useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        setUserId(user.uid);
        const userDoc = await getDoc(doc(db, 'hmos', user.uid));
        if (userDoc.exists()) {
...
const handleLogoUpload = async (file) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateDoc(doc(db, 'hmos', userId), {
 ...
 // Hide alert after 5 seconds
  ...
  return (
    <>
     <HmoHeader page_title={"HMO Account Details"} />
