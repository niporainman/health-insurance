...
function AdminHpAppointments() {
	const { id } = useParams(); // hp firebaseUid
	...
	useEffect(() => {
		...
		const fetchHpName = async () => {
			try {
				const hpRef = doc(db, 'hps', id);
				const hpSnap = await getDoc(hpRef);

				if (hpSnap.exists()) {
					setHpName(hpSnap.data().name || '');
				} else {
					console.warn('HP not found');
				}
			} catch (error) {
	...
	useEffect(() => {
		...
		const filters = [where('hpfirebaseUid', '==', id)];
	...
	return (
		<div className="db-content-box bg-white round-10 mb-25">
			<AdminHeader page_title={`${hpName} Appointments`} />
...
export default AdminHpAppointments;
