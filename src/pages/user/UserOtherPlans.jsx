...
function UserOtherPlans() {
	...
	useEffect(() => {
		document.title =
			import.meta.env.VITE_COMPANY_NAME + ' | Add an existing plan';
	}, []);
	...
	useEffect(() => {
		const fetchHmos = async () => {
			...
			const hmoList = hmoSnapshot.docs.map((doc) => ({
				id: doc.id,
				...doc.data(),
			}));
			setHmos(hmoList);
	...
	const handleChange = (e) => {
		...
		if (id === 'hmoName') {
			...
			const matched = hmos.find(
				(hmo) => hmo.name.toLowerCase() === value.toLowerCase()
			);
			if (matched) {
		...
		if (id === 'planName') {
			...
			if (selectedHmo) {
				const filtered = plans
					...
					.filter((plan) =>
						plan.plan_name.toLowerCase().includes(value.toLowerCase())
					);
				setPlanSuggestions(filtered);

				const matched = filtered.find(
					(plan) => plan.plan_name.toLowerCase() === value.toLowerCase()
				);
	...
	const handleSubmit = async (e) => {
		...
		if (!selectedHmo || !selectedPlan || !formData.enrolleId) {
			return Swal.fire(
				'Missing Info',
				'Please ensure valid HMO, plan, and enrolle number.',
				'warning'
			);
		}
	...
	return (
		<div className="db-content-box bg-white round-10 mb-25">
			...
			<h3>
				Already insured? Add your plan to your Health Insurance Demo Health
				account
			</h3>{' '}
			<br />
			<form className="form-wrapper style-two" onSubmit={handleSubmit}>
				<div className="row">
					<div className="col-xxl-4 col-xl-4 col-md-6 position-relative mb-3">
						<label htmlFor="hmoName" className="fs-13 fw-medium ">
							HMO Name
						</label>
						<input
							type="text"
						...
						{!selectedHmo && formData.hmoName && (
							<p className="text-danger mt-2 fs-12">
								Please select a valid HMO
							</p>
						)}
					</div>

					<div className="col-xxl-4 col-xl-4 col-md-6 position-relative mb-3">
						<label htmlFor="planName" className="fs-13 fw-medium ">
							Insurance Package
						</label>
						<input
							type="text"
						...
						{!selectedPlan && formData.planName && selectedHmo && (
							<p className="text-danger mt-2 fs-12">
								Please select a valid plan for this HMO
							</p>
						)}
					</div>

					<div className="col-xxl-4 col-xl-4 col-md-6 mb-3">
						<label htmlFor="enrolleId" className="fs-13 fw-medium ">
							Enrolle Number
						</label>
						<input
							type="text"
