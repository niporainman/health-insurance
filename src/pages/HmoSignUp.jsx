...
function HmoSignUp() {
	...
	useEffect(() => {
		document.title = import.meta.env.VITE_COMPANY_NAME + ' | HMO Sign Up';
	}, []);
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			await setDoc(doc(db, 'hmos', user.uid), {
	...
	const handleGoogleSignIn = async () => {
		...
		try {
			...
			const userRef = doc(db, 'hmos', user.uid);
	...
	return (
		<>
			<NavigationTop />
			<PageHeader
				page_title="HMO Registration"
				page_title_small="Registration"
				bg="2"
			/>
