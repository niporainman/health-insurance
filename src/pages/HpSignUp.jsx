...
function HpSignUp() {
	...
	useEffect(() => {
		document.title = import.meta.env.VITE_COMPANY_NAME + ' | HP Sign Up';
	}, []);
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			await setDoc(doc(db, 'hps', user.uid), {
	...
	const handleGoogleSignIn = async () => {
		...
		try {
			...
			const userRef = doc(db, 'hps', user.uid);
	...
	return (
		<>
			<NavigationTop />
			<PageHeader
				page_title="HP Registration"
				page_title_small="Register"
				bg="2"
			/>
